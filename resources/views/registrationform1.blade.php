...
<body>
  ...
  <form class="sheet" id="learnerForm" method="POST"
    action="{{ route('registration.step2') }}">
    @csrf
    ...
    <div class="section">
      ...
      <div class="row">
        <div class="field wide"><label>Last Name, Extension (Jr.,
            Sr.)</label><input type="text" name="last_name"
            value="{{ old('last_name') }}"></div>
        <div class="field wide"><label>First Name</label><input type="text"
            name="first_name" value="{{ old('first_name') }}"></div>
        <div class="field wide"><label>Middle Name</label><input type="text"
            name="middle_name" value="{{ old('middle_name') }}"></div>
      </div>

      <div class="subhead"><span class="dot"></span>2.2 Complete Permanent
        Mailing Address</div>
      <div class="row">
        <div class="field wide"><label>Number, Street</label><input
            type="text" name="address_street"
            value="{{ old('address_street') }}"></div>
        <div class="field"><label>Barangay</label><input type="text"
            name="address_barangay" value="{{ old('address_barangay') }}"></div>
      </div>
      <div class="row">
        ...
        <div class="field"><label>Province</label><input type="text"
            name="address_province" value="{{ old('address_province') }}"></div>
        <div class="field"><label>District</label><input type="text"
            name="address_district" value="{{ old('address_district') }}">
        </div>
        <div class="field"><label>Region</label><input type="text"
            name="address_region" value="{{ old('address_region') }}"></div>
      </div>
      <div class="row">
        <div class="field wide"><label>Email Address / Facebook
            Account</label><input type="email" name="email"
            value="{{ old('email') }}"></div>
        <div class="field"><label>Contact No.</label><input type="tel"
            name="contact_no" value="{{ old('contact_no') }}"></div>
        <div class="field"><label>Nationality</label><input type="text"
            name="nationality" value="{{ old('nationality') }}"></div>
      </div>

      <div class="subhead"><span class="dot"></span>2.3 Training Venue
      </div>
      <div class="row">
        <div class="field wide"><input type="text" name="training_venue"
            value="{{ old('training_venue') }}"
            placeholder="Name of training center / venue"></div>
      </div>
    </div>
    ...
    <div class="section">
        ...
        <div>
          ...
          <div class="choice-grid">
            <label class="choice"><input type="radio" name="sex"
                value="Male"
                @if (old('sex') == 'Male') checked @endif> Male</label>
            <label class="choice"><input type="radio" name="sex"
                value="Female"
        ...
        <div>
          ...
          <div class="choice-grid">
            <label class="choice"><input type="radio"
                name="employment_status" value="Employed"
                @if (old('employment_status') == 'Employed') checked @endif>
              Employed</label>
            <label class="choice"><input type="radio"
                name="employment_status" value="Unemployed"
                @if (old('employment_status') == 'Unemployed') checked @endif>
              Unemployed</label>
          </div>
        </div>
      </div>
      ...
      <div class="choice-grid" style="margin-bottom:6px;">
        <label class="choice"><input type="radio" name="civil_status"
            value="Single" @if (old('civil_status') == 'Single') checked @endif>
          Single</label>
        <label class="choice"><input type="radio" name="civil_status"
            value="Married" @if (old('civil_status') == 'Married') checked @endif>
          Married</label>
        <label class="choice"><input type="radio" name="civil_status"
            value="Widowed" @if (old('civil_status') == 'Widowed') checked @endif>
          Widowed</label>
        <label class="choice"><input type="radio" name="civil_status"
            value="Separated" @if (old('civil_status') == 'Separated') checked @endif>
          Separated</label>
        <label class="choice"><input type="radio" name="civil_status"
            value="Solo Parent"
            @if (old('civil_status') == 'Solo Parent') checked @endif> Solo
          Parent</label>
      </div>
      ...
      <div class="row">
        <div class="field"><label>Month of Birth</label><input type="text"
            name="birth_month" value="{{ old('birth_month') }}"
            placeholder="e.g. January"></div>
        <div class="field small"><label>Day of Birth</label><input
            type="text" name="birth_day" value="{{ old('birth_day') }}"
            placeholder="DD"></div>
        <div class="field small"><label>Year of Birth</label><input
            type="text" name="birth_year" value="{{ old('birth_year') }}"
            placeholder="YYYY"></div>
        <div class="field small"><label>Age</label><input type="text"
            name="age" value="{{ old('age') }}"></div>
      </div>
      ...
      <div class="row">
        <div class="field"><label>City / Municipality</label><input
            ...
            value="{{ old('birthplace_city') }}"></div>
        <div class="field"><label>Province</label><input type="text"
            name="birthplace_province"
            value="{{ old('birthplace_province') }}"></div>
        <div class="field"><label>Region</label><input type="text"
            name="birthplace_region" value="{{ old('birthplace_region') }}">
        </div>
      </div>

      <div class="subhead"><span class="dot"></span>3.6 Educational
        Attainment Before the Training (Trainee)</div>
      @php
        $eduOptions = [
            'No Grade Completed',
        ...
        @foreach ($eduOptions as $opt)
          <label class="choice">
            <input type="radio" name="education_attainment"
              value="{{ $opt }}"
              @if (old('education_attainment') == $opt) checked @endif>
            {{ $opt }}
          </label>
        @endforeach
      </div>

      <div class="subhead"><span class="dot"></span>3.7 Parent / Guardian
      </div>
      <div class="row">
        <div class="field wide"><label>Name</label><input type="text"
            name="guardian_name" value="{{ old('guardian_name') }}"></div>
      </div>
      <div class="row">
        <div class="field wide"><label>Complete Permanent Mailing
            Address</label><input type="text" name="guardian_address"
            value="{{ old('guardian_address') }}"></div>
      </div>
    </div>

    <div class="footer-note">
      <span>Please review all entries before proceeding. Fields marked with a
