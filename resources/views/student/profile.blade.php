...
@section('content')
  <div class="pf-wrap">
    ...
    {{-- Alerts --}}
    @if (session('success'))
      <div class="pf-alert success"><i class="fa fa-check-circle"></i>
        {{ session('success') }}</div>
    @endif
    @if (session('error'))
      <div class="pf-alert error"><i class="fa fa-circle-exclamation"></i>
        {{ session('error') }}</div>
    @endif
    ...
    <div class="pf-card">
      ...
      @forelse($enrollments as $enrollment)
        <div class="pf-enroll-item">
          <div class="pf-enroll-title">{{ $enrollment->course->title ?? 'N/A' }}
          </div>
          <div class="pf-progress-bg">
            <div class="pf-progress-fill"
          ...
          <div class="pf-enroll-meta">
            <span>
              ...
              @endphp
              <span
                style="color:{{ $color }};font-weight:600;">{{ ucfirst($s) }}</span>
            </span>
    ...
    <div class="pf-card">
      ...
      @if ($completedCount > 0)
        @foreach ($enrollments->where('status', 'completed') as $enrollment)
          <div
            style="display:flex;align-items:center;gap:12px;padding:10px 0;border-bottom:1px solid #f0f4f0;">
            <div
              style="width:36px;height:36px;border-radius:8px;background:#e8f5e9;display:flex;align-items:center;justify-content:center;">
              <i class="fa fa-certificate"
                style="color:#025628;font-size:16px;"></i>
            </div>
            <div>
              <div style="font-size:13px;font-weight:600;color:#1a1a1a;">
                {{ $enrollment->course->title ?? 'Course' }}</div>
              <div style="font-size:11px;color:#aaa;">
                Completed
                {{ $enrollment->completed_at ? \Carbon\Carbon::parse($enrollment->completed_at)->format('M d, Y') : '' }}
              </div>
            </div>
          </div>
        @endforeach
