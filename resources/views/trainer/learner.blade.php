...
@section('content')
    ...
    <table class="trainee-data-table" id="traineeTable">
      ...
      <tbody>
        <tr>
          ...
          <td class="action-icons">
            <i class="fas fa-eye view"
              onclick="openProfile('Bong, Marcos','2023-2-0018','Salitran II, Dasmariñas, Cavite','Nail Care')"></i>
        ...
        <tr>
          ...
          <td class="action-icons">
            <i class="fas fa-eye view"
              onclick="openProfile('Ramos, Roshian','2023-2-0019','Sabang, Dasmariñas, Cavite','Candle Making')"></i>
  ...
  <div class="bottom-controls">
    <button class="export-pill-btn" onclick="exportToExcel()">Export to
      Excel</button>
  </div>
  ...
  <div id="profileModal" class="modal-overlay">
    <div class="modal-box profile-modal">
      <h2 class="modal-title"
        style="color:var(--primary-green);margin-bottom:12px;">Trainee Profile
      </h2>
      <div class="profile-photo-circle"><i class="fas fa-user"></i></div>
      <h2 id="profName"
        style="color:var(--primary-green);font-size:22px;margin:8px 0 2px;">Name
        Here</h2>
      <p id="profID" style="color:#666;font-size:13px;margin-bottom:14px;">ID:
        0000</p>
      <hr class="modal-divider">
      <div style="text-align:left;padding:0 8px;">
        <div style="margin-bottom:16px;">
          <span class="section-badge">Personal Info</span>
          <p style="font-size:14px;margin:6px 0;color:#444;">
            <strong>Address:</strong> <span id="profAddr">—</span></p>
          <p style="font-size:14px;margin:6px 0;color:#444;">
            <strong>Course:</strong> <span id="profCourse">—</span></p>
        </div>
        <div>
          <span class="section-badge">Documents</span>
          <p style="font-size:13px;color:#2e7d32;margin:6px 0;"><i
              class="fas fa-check-circle"></i> 2x2 Picture</p>
          <p style="font-size:13px;color:#c62828;margin:6px 0;"><i
              class="fas fa-times-circle"></i> Valid ID (Missing)</p>
        </div>
      </div>
      <div class="modal-footer-centered">
        <button class="close-profile-btn" onclick="closeProfile()">Close
          Profile</button>
      </div>
    </div>
  </div>
    ...
    document.querySelectorAll('.filter-dropdown').forEach(select => {
      select.addEventListener('change', function() {
        ...
        const status = document.getElementById('filterStatus').value
          .toLowerCase();
        document.querySelectorAll('#traineeTable tbody tr').forEach(
        row => {
          const show = (course === "" || row.cells[2].innerText
              .toLowerCase().includes(course)) &&
            (brgy === "" || row.cells[3].innerText.toLowerCase()
              .includes(brgy)) &&
            (status === "" || row.cells[4].innerText.toLowerCase()
              .includes(status));
