...
.mc-badge.inactive { background: #aaa; }
.mc-card-body { padding: 20px; text-align: center; }
.mc-card-title {
    font-size: 16px;
    font-weight: 700;
    color: #1a1a1a;
...
.mc-card-meta i { color: #025628; width: 14px; font-size: 13px; }
.mc-trainer-row {
    font-size: 12px;
    font-weight: 600;
    color: #025628;
...
.mc-progress-fill { background: #025628; height: 100%; }
...
.mc-btn.primary { background: #025628; color: #fff; }
.mc-btn.primary:hover { background: #013d1c; color: #fff; }
.mc-btn.outline { background: #fff; color: #025628; border: 2px solid #025628; }
.mc-btn.outline:hover { background: #e8f5e9; }
...
.mc-detail-icon i { font-size: 14px; color: #025628; }
.mc-detail-label { font-size: 11px; color: #aaa; margin-bottom: 2px; text-transform: uppercase; letter-spacing: 0.05em; }
.mc-detail-value { font-size: 14px; color: #1a1a1a; font-weight: 500; }
...
.mc-empty i { font-size: 48px; opacity: 0.3; display: block; margin-bottom: 14px; color: #025628; }
    ...
    <div class="mc-course-card">
        ...
        <div class="mc-card-body">
            ...
            <div class="mc-card-meta">
                <p><i class="fa fa-calendar-day"></i> Duration: {{ $course->duration ?? 'TBA' }}</p>
                <p><i class="fa fa-users"></i> Slots: {{ $totalStudents }}/{{ $course->slots ?? '—' }}</p>
                <p><i class="fa fa-clock"></i> Schedule: {{ $course->schedule ?? 'TBA' }}</p>
                <p><i class="fa fa-location-dot"></i> {{ Str::limit($course->location ?? 'TBA', 38) }}</p>
            </div>
            ...
            <div class="mc-progress-bg">
                <div class="mc-progress-fill" style="width: {{ $course->progress ?? 0 }}%"></div>
            </div>
        ...
        <div class="mc-modal-body">
            ...
            <div class="mc-detail-row">
                ...
                <div>
                    ...
                    <div class="mc-detail-value" id="cd-description" style="font-size:13px;color:#555;font-weight:400;line-height:1.6;"></div>
                </div>
            </div>
            <div class="mc-detail-row">
...
<div id="contentModal" class="mc-modal">
    <div class="mc-modal-content wide">
        <div class="mc-modal-header">
            <h3><i class="fa fa-layer-group"></i> <span id="contentModalCourseTitle"></span></h3>
        ...
        <div class="mc-modal-body" style="padding-bottom:0;">
            ...
            <div id="content-tab-quizzes" style="display:none;">
                ...
                <div id="quizListContainer" style="display:flex;flex-direction:column;gap:8px;max-height:100%;overflow-y:visible;">
        ...
        <div class="mc-modal-footer">
            <button class="mc-btn-close" onclick="closeContentModal()">Close</button>
        </div>
    </div>
</div>
...
function openCourseDetails(title, description, duration, slots, schedule, location, sector, status) {
    document.getElementById('cd-title').textContent       = title;
    document.getElementById('cd-description').textContent = description || 'No description available.';
...
function switchContentTab(tab) {
    ...
    document.getElementById('tab-btn-modules').className = 'mc-tab-btn' + (isMod ? ' active' : '');
    document.getElementById('tab-btn-quizzes').className = 'mc-tab-btn' + (!isMod ? ' active' : '');
...
function addQuiz() {
    ...
    fetch('/trainer/quiz', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': csrfToken },
        body: JSON.stringify({ course_id: _contentCourseId, module_id: moduleId, title, passing_score: passing, time_limit: time })
    })
    .then(r => r.json())
    .then(data => {
        if (data.success) {
...
function addQuestion(quizId) {
    ...
    if (!question || !a || !b || !c || !d) { alert('Punan ang lahat ng fields.'); return; }
    ...
    .then(data => {
        if (data.success) {
            ['qtext','qa','qb','qc','qd'].forEach(id => document.getElementById(`${id}-${quizId}`).value = '');
