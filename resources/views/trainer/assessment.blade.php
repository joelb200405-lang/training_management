...
.as-stat.urgent::before { background: #A32D2D; }
.as-stat-val {
    font-size: 26px;
    font-weight: 700;
    color: #025628;
...
.as-stat.urgent .as-stat-val { color: #A32D2D; }
.as-stat-label {
    font-size: 12px;
    color: #888;
    font-weight: 500;
...
.as-filter:focus { border-color: #025628; }
...
.as-table tbody tr:hover { background: #f9fdf9; }
.as-table td {
    padding: 12px 16px;
    font-size: 13px;
    color: #333;
...
.as-badge.complete   { background: #e8f5e9; color: #025628; }
.as-badge.incomplete { background: #FCEBEB; color: #A32D2D; }
.as-badge.competent     { background: #e8f5e9; color: #025628; }
.as-badge.not-competent { background: #fff8e1; color: #854F0B; }

/* ── Action icons ─────────────────────────────────────────── */
.as-actions { display: flex; gap: 8px; align-items: center; }
.as-icon {
    width: 30px;
    height: 30px;
    border-radius: 8px;
...
.as-icon.view   { background: #e8f5e9; color: #025628; }
.as-icon.edit   { background: #e8f0fe; color: #3B5BDB; }
.as-icon.delete { background: #FCEBEB; color: #A32D2D; }
.as-icon:hover { opacity: 0.8; }
...
.as-btn.primary { background: #025628; color: #fff; }
.as-btn.primary:hover { background: #013d1c; }
.as-btn.outline { background: #fff; color: #025628; border: 1.5px solid #025628; }
.as-btn.outline:hover { background: #f0f7f2; }
...
.as-empty i { font-size: 36px; display: block; margin-bottom: 10px; opacity: 0.3; color: #025628; }
...
.as-modal-close:hover { color: #A32D2D; }
.as-modal-body { padding: 20px 24px; }
.as-modal-footer {
    padding: 14px 24px;
...
.as-form-group input:focus { border-color: #025628; }
...
.as-detail-label { color: #888; }
.as-detail-val   { font-weight: 600; color: #1a1a1a; }

/* Top performers table */
.as-top-table { width: 100%; border-collapse: collapse; }
.as-top-table th {
    padding: 10px 12px;
    font-size: 11px;
    font-weight: 700;
...
.as-top-table tr:last-child td { border-bottom: none; }
...
<div class="as-wrap">
    ...
    <div class="as-page-sub">Quiz results and competency records ng iyong mga trainees.</div>
    ...
    <div class="as-stats">
        ...
        <div class="as-stat urgent">
            ...
            <span class="as-stat-link" onclick="openModal('topModal')">View all →</span>
        </div>
    </div>
            ...
            <tbody>
                @forelse($results as $result)
                    <tr>
                        ...
                        <td><strong>{{ $result->user->lastname }}, {{ $result->user->firstname }}</strong></td>
                        <td>{{ $result->quiz->title }}</td>
                        <td>
                            <span class="as-badge {{ $result->status === 'passed' ? 'complete' : 'incomplete' }}">
                                {{ $result->score }}/{{ $result->total_items }} ({{ $result->percentage }}%)
                            </span>
                        </td>
                        <td>
                            <span class="as-badge {{ $result->status === 'passed' ? 'competent' : 'not-competent' }}">
...
<div id="detailsModal" class="as-modal">
    <div class="as-modal-content">
        <div class="as-modal-header">
            ...
            <button class="as-modal-close" onclick="closeModal('detailsModal')">&times;</button>
        </div>
        <div class="as-modal-body">
            <div style="display:flex; align-items:center; gap:14px; margin-bottom:16px; padding-bottom:16px; border-bottom:1px solid #f0f4f0;">
                <div style="width:46px; height:46px; border-radius:50%; background:#e8f5e9; display:flex; align-items:center; justify-content:center; font-size:18px; color:#025628; flex-shrink:0;">
                    <i class="fa fa-user"></i>
                </div>
                <div>
                    <div style="font-size:15px; font-weight:700; color:#1a1a1a;" id="det-name">—</div>
                    <div style="font-size:12px; color:#888;" id="det-id">ID: —</div>
                    <<div style="font-size:13px; color:#025628; font-weight:700; margin-top:4px;" id="det-score">—</div>
                </div>
            </div>
            <div style="font-size:11px; font-weight:700; color:#025628; text-transform:uppercase; letter-spacing:.04em; margin-bottom:8px;">
                Assessment Scores
            </div>
            ...
            <div style="font-size:11px; font-weight:700; color:#025628; text-transform:uppercase; letter-spacing:.04em; margin:14px 0 8px;">
                Activity Status
            </div>
            <div class="as-detail-row">
                <span class="as-detail-label"><i class="fa fa-check-circle" style="color:#025628;"></i> Lesson 1 Modules</span>
                <span class="as-badge complete">Done</span>
            </div>
            <div class="as-detail-row">
                <span class="as-detail-label"><i class="fa fa-times-circle" style="color:#A32D2D;"></i> Final Project Draft</span>
                <span class="as-badge incomplete">Missing</span>
            </div>
        </div>
        <div class="as-modal-footer">
            <button class="as-btn outline" onclick="closeModal('detailsModal')">Close</button>
        </div>
    </div>
</div>
...
<div id="gradeModal" class="as-modal">
    <div class="as-modal-content">
        <div class="as-modal-header">
            ...
            <button class="as-modal-close" onclick="closeModal('gradeModal')">&times;</button>
        </div>
        <div class="as-modal-body">
            ...
            <div class="as-form-group">
                ...
                <input type="number" id="gradeScore" placeholder="Enter score" min="0" max="100" oninput="calcResult()">
            </div>
        ...
        <div class="as-modal-footer">
            <button class="as-btn outline" onclick="closeModal('gradeModal')">Cancel</button>
...
<div id="topModal" class="as-modal">
    <div class="as-modal-content">
        <div class="as-modal-header">
            ...
            <button class="as-modal-close" onclick="closeModal('topModal')">&times;</button>
        </div>
                    ...
                    <tbody>
                        @forelse($topPerformers as $i => $tp)
                            <tr>
                                ...
                                <td><strong>{{ $tp->user->lastname }}, {{ $tp->user->firstname }}</strong></td>
                                <td>{{ $course->title }}</td>
                                <td><span class="as-badge competent">{{ $tp->avg_score }}%</span></td>
                            </tr>
                        @empty
                            <tr>
        ...
        <div class="as-modal-footer">
            <button class="as-btn outline" onclick="closeModal('topModal')">Close</button>
        </div>
    </div>
</div>
...
function openDetailsModal(name, id, quiz, score, total, percentage) {
    ...
    document.getElementById('det-score').textContent  = score + '/' + total + ' (' + percentage + '%)';
...
function calcResult() {
    ...
    if (isNaN(score)) { text.textContent = '—'; text.style.color = '#025628'; return; }
...
function applyFilters() {
    ...
    document.querySelectorAll('#assessmentTable tbody tr').forEach(row => {
        const show = (course     === '' || row.cells[2].textContent.toLowerCase().includes(course))     &&
                     (result     === '' || row.cells[3].textContent.toLowerCase().includes(result))     &&
                     (competency === '' || row.cells[4].textContent.toLowerCase().includes(competency));
