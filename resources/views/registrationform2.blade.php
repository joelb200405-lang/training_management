...
<style>
  ...
  *{box-sizing:border-box;}
  ...
  .section-head .hint{
    font-family:"Trebuchet MS", sans-serif; font-size:11px; font-style:italic;
  ...
  .inline-input{
    ...
    font-family:"Trebuchet MS", sans-serif; font-size:13px; padding:2px 4px; width:160px;
  }
  .inline-input:focus{ outline:none; border-bottom-color:var(--navy); }
  .disclaimer-box{
    background:var(--accent-soft); border:1px solid var(--line); border-radius:var(--radius);
    padding:14px 16px; font-size:14px; line-height:1.6; margin-bottom:12px;
  }
  ...
  .sig-line{ border-bottom:1px solid var(--ink); height:36px; }
  .sig-caption{
    font-family:"Trebuchet MS", sans-serif; font-size:11px; color:var(--muted);
    text-transform:uppercase; letter-spacing:.03em; text-align:center;
  }
  .photo-thumb-row{ display:flex; gap:20px; flex-wrap:wrap; margin-top:16px; }
  .photo-thumb{
    width:100px; height:110px; border:2px dashed var(--line); border-radius:var(--radius);
    display:flex; align-items:center; justify-content:center; flex-direction:column;
    color:var(--muted); font-family:"Trebuchet MS", sans-serif; font-size:10.5px;
    text-align:center; cursor:pointer; background:var(--accent-soft); overflow:hidden; flex-shrink:0;
  }
  .photo-thumb img{ width:100%; height:100%; object-fit:cover; }
  .photo-thumb input{ display:none; }
  .certify-note{
    font-family:"Trebuchet MS", sans-serif; font-size:12px; font-style:italic;
...
<form class="sheet" id="learnerForm2" method="POST" action="{{ route('registration.submit') }}">
  @csrf
  ...
  <div class="section">
    ...
    @php
      $classifications = [
        ...
        'Displaced Workers','Drug Dependents Surrenderees/Surrenderers',
        'Family Members of AFP and PNP Wounded-in-Action','Family Members of AFP and PNP Killed-in-Action',
        'Farmers and Fishermen','Indigenous People & Cultural Communities','Industry Workers',
        'Inmates and Detainees','MILF Beneficiary','Out-of-School Youth',
        'Overseas Filipino Workers (OFW) Dependents','RCEF-RESP',
        'Rebel Returnees/Decommissioned Combatants','Returning/Repatriated OFW',
        'Student','TESDA Alumni','TVET Trainers','Uniformed Personnel',
    ...
    <div class="check-grid">
      @foreach($classifications as $i => $c)
        <label class="choice">
          <input type="checkbox" name="classification[]" value="{{ $c }}"
            @if(is_array(old('classification')) && in_array($c, old('classification'))) checked @endif> {{ $c }}
        </label>
      @endforeach
      <label class="choice">
        <input type="checkbox" name="classification[]" value="Others" id="classOthersCheck">
        Others (please specify):
        <input type="text" name="classification_other" class="inline-input" value="{{ old('classification_other') }}">
      </label>
    </div>
  </div>
  ...
  <div class="section">
    <div class="section-head">
      ...
      <h2>Type of Disability <span class="hint">(for Persons with Disability only — to be filled up by TESDA personnel)</span></h2>
    </div>
    @php
      $disabilityTypes = [
        'Mental/Intellectual Disability','Hearing Disability','Psychosocial Disability',
        'Visual Disability','Speech Impairment','Disability Due to Chronic Illness',
        'Learning Disability','Orthopedic (Musculoskeletal) Disability',
      ];
    @endphp
    <div class="check-grid">
      @foreach($disabilityTypes as $d)
        <label class="choice">
          <input type="checkbox" name="disability_type[]" value="{{ $d }}"
            @if(is_array(old('disability_type')) && in_array($d, old('disability_type'))) checked @endif> {{ $d }}
        </label>
      @endforeach
      <label class="choice">
        Multiple Disabilities, specify:
        <input type="text" name="disability_multiple_specify" class="inline-input" value="{{ old('disability_multiple_specify') }}">
      </label>
    </div>
  </div>
  ...
  <div class="section">
    <div class="section-head">
      ...
      <h2>Causes of Disability <span class="hint">(for Persons with Disability only — to be filled up by TESDA personnel)</span></h2>
    </div>
    <div class="choice-grid">
      <label class="choice"><input type="radio" name="disability_cause" value="Congenital/Inborn" @if(old('disability_cause')=='Congenital/Inborn') checked @endif> Congenital/Inborn</label>
      <label class="choice"><input type="radio" name="disability_cause" value="Illness" @if(old('disability_cause')=='Illness') checked @endif> Illness</label>
      <label class="choice"><input type="radio" name="disability_cause" value="Injury" @if(old('disability_cause')=='Injury') checked @endif> Injury</label>
      <label class="choice">
        Others, please specify:
        <input type="text" name="disability_cause_other" class="inline-input" value="{{ old('disability_cause_other') }}">
      </label>
    </div>
  </div>
  ...
  <div class="section">
    ...
    <div class="row">
      <div class="field wide"><input type="text" name="course_name" value="{{ old('course_name') }}"></div>
    </div>
  </div>
  ...
  <div class="section">
    ...
    <div class="row">
      <div class="field wide">
        ...
        <input type="text" name="scholarship_package" value="{{ old('scholarship_package') }}">
      </div>
    </div>
  </div>
  ...
  <div class="section">
    ...
    <div class="disclaimer-box">
      I hereby allow TESDA to use/post my contact details, name, email, cellphone/landline nos.
      and other information I provided which may be used for processing of my scholarship
      application, for employment opportunities and for the survey of TESDA programs.
    </div>
    <div class="choice-grid">
      <label class="choice"><input type="radio" name="privacy_consent" value="Agree" @if(old('privacy_consent')=='Agree') checked @endif> Agree</label>
      <label class="choice"><input type="radio" name="privacy_consent" value="Disagree" @if(old('privacy_consent')=='Disagree') checked @endif> Disagree</label>
    </div>
  </div>
  ...
  <div class="section">
    ...
    <p class="certify-note">This is to certify that the information stated above is true and correct.</p>

    <div class="sig-grid" style="margin-top:18px;">
      <div class="sig-block">
        <div class="sig-line"></div>
        <div class="sig-caption">Applicant's Signature Over Printed Name</div>
      </div>
      <div class="sig-block">
        <label>Date Accomplished</label>
        <input type="date" name="date_accomplished" value="{{ old('date_accomplished') }}">
      </div>
    </div>
    ...
    <div class="sig-grid" style="margin-top:22px;">
      <div class="sig-block">
        <div class="sig-line"></div>
        <div class="sig-caption">Noted by: Registrar / School Administrator<br>(Signature Over Printed Name)</div>
      </div>
      <div class="sig-block">
    ...
    <div class="photo-thumb-row">
      <label class="photo-thumb" for="idPhoto1x1">
        <span id="photoText1x1">1x1 picture<br>taken within<br>the last 6 months</span>
        <img id="photoPreview1x1" style="display:none;">
        <input type="file" id="idPhoto1x1" name="photo_1x1" accept="image/*">
      </label>
  ...
  <div class="footer-note">
    <span>Please review all entries before submitting. This completes the TESDA Registration Form (MIS 03-01).</span>
  </div>
  <div class="footer-note" style="padding-top:0;">
    <div class="btn-row">
      <a href="{{ route('registration.step1') }}" class="btn btn-outline">&larr; Back</a>
